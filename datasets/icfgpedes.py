...
class ICFGPEDES(BaseDataset):
    ...
    def __init__(self, root='', verbose=True, use_occlusion=False):
        ...
        if self.use_occlusion:
            self.anno_path = op.join(self.dataset_dir, 'ICFG-PEDES_occlusion.json')
        else:
    ...
    def _process_anno(self, annos: List[dict], training=False):
        ...
        if training:
            ...
            for anno in annos:
                ...
                if not img_path.startswith('imgs/'):
                    img_path = op.join('imgs', img_path)
                img_path = op.join(self.dataset_dir, img_path)
