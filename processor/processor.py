...
def split_prob(prob, threshld):
    if prob.min() > threshld:
        ...
        # If all probabilities are larger than threshold, enforce 1/100 data to be unlabeled
        threshld = np.sort(prob)[len(prob)//100]
    pred = (prob > threshld)
    return (pred+0)

def get_loss(model, data_loader):
    logger = logging.getLogger("HSRNet.train")
    model.eval()
    device = "cuda"
    ...
    for i, batch in enumerate(data_loader):
        # Only move tensor data to device, skip string data like img_paths
    ...
    logger.info('Analyzing loss distributions with Gaussian Mixture Models...') 
 
    if  model.args.dataset_name=='RSTPReid':
...
def do_train(start_epoch, args, model, train_loader, evaluator, optimizer,
    ...
    meters = {
        "loss": AverageMeter(),
        "vsel_loss": AverageMeter(),
        "tsel_loss": AverageMeter(),
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        for n_iter, batch in enumerate(train_loader):
            ...
            ret = model(batch)
            loss = ret['vsel_loss'] + ret['tsel_loss'] + ret['id_loss']
            if 'recon_loss' in ret:
                loss += ret['recon_loss']
            ...
            meters['vsel_loss'].update(ret['vsel_loss'].item(), batch_size)
            meters['tsel_loss'].update(ret['tsel_loss'].item(), batch_size)
            meters['id_loss'].update(ret['id_loss'].item(), batch_size)
            meters['img_acc'].update(ret['img_acc'].item(), batch_size)
            meters['txt_acc'].update(ret['txt_acc'].item(), batch_size)
            if 'recon_loss' in ret:
                meters['recon_loss'].update(ret['recon_loss'].item(), batch_size)
...
def do_inference(model, test_img_loader, test_txt_loader):
    ...
    logger = logging.getLogger("HSRNet.test")
